  ...
  return (
    <div style={s.container}>
      ...
      <div style={s.content}>
        ...
        {activeTab === 'qr' && selectedEvent && (<><div style={s.title}>QR Codes: {selectedEvent.name}</div><div style={{...s.card, marginBottom: 16, backgroundColor: `${colors.champagne}10`, border: `1px solid ${colors.champagne}`}}><div style={{ fontSize: 12, color: colors.champagne, marginBottom: 8, fontWeight: 600 }}>💡 Nou! URL-uri simplificate</div><div style={{ fontSize: 11, color: colors.textMuted }}>Format: /m/venue/MASA</div><div style={{ fontSize: 11, color: colors.textMuted, marginTop: 4 }}>Exemplu: <strong>/m/testing/VIP1</strong></div></div>{eventTables.map(t => { const venueSlug = 'testing'; const tableSlug = t.table_number.replace(/\s+/g, ''); const newUrl = `/m/${venueSlug}/${tableSlug}`; const oldUrl = `/order/${selectedEvent.id}/${t.id}`; return (<div key={t.id} style={{...s.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><div><div style={{ fontWeight: 500 }}>{t.table_number}</div><div style={{ fontSize: 11, color: colors.success, marginBottom: 2 }}>✨ {newUrl}</div><div style={{ fontSize: 10, color: colors.textMuted }}>(legacy: {oldUrl})</div></div><div style={{ display: 'flex', gap: 8 }}><button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}${newUrl}`); alert('Copiat!') }} style={{...s.btnSm, backgroundColor: colors.champagne, color: colors.noir}}>📋 Nou</button><button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}${oldUrl}`); alert('Copiat!') }} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>Legacy</button></div></div>) })}</>)}
