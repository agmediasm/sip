  ...
  return (
    <div style={s.container}>
      ...
      <div style={s.content}>
        ...
        {activeTab === 'qr' && selectedEvent && (<><div style={s.title}>QR Codes: {selectedEvent.name}</div><div style={{...s.card, marginBottom: 16, backgroundColor: `${colors.champagne}10`, border: `1px solid ${colors.champagne}`}}><div style={{ fontSize: 12, color: colors.champagne, marginBottom: 8, fontWeight: 600 }}>📱 Link-uri pentru mese</div><div style={{ fontSize: 11, color: colors.textMuted }}>🟢 <strong>LIVE</strong> - folosește venue-ul real (setează slug în Supabase)</div><div style={{ fontSize: 11, color: colors.textMuted, marginTop: 4 }}>🧪 <strong>TEST</strong> - funcționează mereu pentru testare</div></div>{eventTables.map(t => { const venueSlug = 'nuba'; const tableSlug = t.table_number.replace(/\s+/g, ''); const liveUrl = `/m/${venueSlug}/${tableSlug}`; const testUrl = `/m/testing/${tableSlug}`; return (<div key={t.id} style={{...s.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><div><div style={{ fontWeight: 500, marginBottom: 6 }}>{t.table_number}</div><div style={{ fontSize: 11, color: colors.success, marginBottom: 2 }}>🟢 {liveUrl}</div><div style={{ fontSize: 11, color: colors.warning }}>🧪 {testUrl}</div></div><div style={{ display: 'flex', gap: 8 }}><button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}${liveUrl}`); alert('Link LIVE copiat!') }} style={{...s.btnSm, backgroundColor: colors.success, color: '#fff'}}>LIVE</button><button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}${testUrl}`); alert('Link TEST copiat!') }} style={{...s.btnSm, backgroundColor: colors.warning, color: colors.noir}}>TEST</button></div></div>) })}</>)}
