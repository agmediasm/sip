...
export default function ManagerDashboard() {
  ...
  const [waiterForm, setWaiterForm] = useState({ name: '', phone: '', pin: '' })
  ...
  const loadMenuOnly = async () => { 
    const [menuRes, catsRes] = await Promise.all([getAllMenuItems(VENUE_ID), supabase.from('categories').select('*').eq('venue_id', VENUE_ID).eq('is_active', true).order('sort_order')])
    if (menuRes.data) setMenuItems(menuRes.data)
    if (catsRes.data) {
      const uniqueCats = []
      const seenNames = new Set()
  ...
  const handleDeleteProduct = async (id) => { if (!confirm('Ștergi?')) return; await deleteMenuItem(id); loadMenuOnly() }
  ...
  const handleBulkDelete = async () => { if (selectedItems.size === 0) return; if (!confirm(`Ștergi ${selectedItems.size} produse?`)) return; await bulkDeleteMenuItems(Array.from(selectedItems)); setSelectedItems(new Set()); setSelectMode(false); loadMenuOnly() }
  ...
  const selectAllInCategory = (catName) => { const catItems = menuItems.filter(m => m.categories?.name?.toLowerCase() === catName.toLowerCase()); const allSelected = catItems.every(m => selectedItems.has(m.id)); const newSet = new Set(selectedItems); if (allSelected) { catItems.forEach(m => newSet.delete(m.id)) } else { catItems.forEach(m => newSet.add(m.id)) }; setSelectedItems(newSet) }
  ...
  const handleDeleteCategory = async (id) => { if (menuItems.filter(m => m.category_id === id).length) { alert('Categorie cu produse!'); return }; if (!confirm('Ștergi?')) return; await deleteCategory(id); loadMenuOnly() }
  ...
  const handleDeleteMenuTemplate = async (id) => { if (!confirm('Ștergi?')) return; await deleteMenuTemplate(id); if (defaultTemplateId === id) setDefaultTemplateId(null); const { data } = await getMenuTemplates(VENUE_ID); if (data) setMenuTemplates(data) }
  ...
  const handleCreateWaiter = async () => { 
    ...
    const pin = waiterForm.pin || generatePin()
    await createWaiter({ ...waiterForm, pin, venue_id: VENUE_ID })
    setShowWaiterModal(false)
    setWaiterForm({ name: '', phone: '', pin: '' })
    setEditingWaiter(null)
    loadWaiters() 
  }
  const handleUpdateWaiter = async () => {
    if (!editingWaiter || !waiterForm.name) return
    await supabase.from('waiters').update({ name: waiterForm.name, phone: waiterForm.phone, pin: waiterForm.pin }).eq('id', editingWaiter.id)
    setShowWaiterModal(false)
    setWaiterForm({ name: '', phone: '', pin: '' })
    setEditingWaiter(null)
    loadWaiters()
  }
  const openEditWaiter = (w, e) => {
    e.stopPropagation()
    setEditingWaiter(w)
    setWaiterForm({ name: w.name, phone: w.phone, pin: w.pin || '' })
    setShowWaiterModal(true)
  }
  const resetWaiterPin = async (w, e) => {
    e.stopPropagation()
    const newPin = generatePin()
    await supabase.from('waiters').update({ pin: newPin }).eq('id', w.id)
    alert(`Noul PIN pentru ${w.name}: ${newPin}`)
    loadWaiters()
  }
  const toggleShowPin = (id) => setShowPin(prev => ({ ...prev, [id]: !prev[id] }))
  ...
  const handleBulkDeleteTables = async () => {
    ...
    if (!confirm(`Ștergi ${selectedTables.size} mese?`)) return
  ...
  const renderGrid = (forRes = false) => {
    ...
    const stageStyle = { backgroundColor: `${colors.champagne}15`, border: `2px solid ${colors.champagne}`, borderRadius: 8, padding: '8px 0', textAlign: 'center', fontSize: 10, fontWeight: 700, color: colors.champagne, letterSpacing: 2, marginBottom: 8 }
    
    return (
      <div style={{ overflowX: 'auto' }}>
        <div style={s.zoneTabs}><button onClick={() => setActiveZone('front')} style={{...s.zoneTab, backgroundColor: activeZone === 'front' ? colors.champagne : 'transparent', color: activeZone === 'front' ? colors.noir : colors.textMuted}}>🎭 Față</button><button onClick={() => setActiveZone('back')} style={{...s.zoneTab, backgroundColor: activeZone === 'back' ? colors.champagne : 'transparent', color: activeZone === 'back' ? colors.noir : colors.textMuted}}>🎪 Spate</button></div>
        
        {/* Stage indicator for front zone */}
        {activeZone === 'front' && <div style={stageStyle}>🎭 SCENĂ</div>}
        ...
        {!forRes && <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ flex: 1 }} />
          <button onClick={() => setTableSelectMode(!tableSelectMode)} style={{...s.btnSm, backgroundColor: tableSelectMode ? colors.warning : 'transparent', color: tableSelectMode ? colors.noir : colors.textMuted, border: `1px solid ${tableSelectMode ? colors.warning : colors.border}`}}>{tableSelectMode ? '✓ Selectare' : '☐ Selectare'}</button>
        ...
        {tableSelectMode && selectedTables.size > 0 && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, padding: 10, backgroundColor: `${colors.warning}15`, borderRadius: 8, alignItems: 'center' }}>
            <span style={{ fontSize: 12, color: colors.warning, fontWeight: 600 }}>{selectedTables.size} mese selectate</span>
            <div style={{ flex: 1 }} />
            <button onClick={handleBulkDeleteTables} style={{...s.btnSm, backgroundColor: colors.error, color: '#fff'}}>🗑️ Șterge</button>
            <button onClick={() => { setSelectedTables(new Set()); setTableSelectMode(false) }} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>×</button>
          </div>
        )}
        
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${curCols}, ${cellSize}px)`, gap, padding: 10, backgroundColor: colors.noir, border: `1px solid ${colors.border}`, borderRadius: 8, width: 'fit-content' }}>{Array.from({ length: curRows }).map((_, row) => Array.from({ length: curCols }).map((_, col) => { const t = curZoneTables.find(t => t.grid_row === row && t.grid_col === col); if (forRes && !t) return <div key={`${row}-${col}`} style={{ width: cellSize, height: cellSize }} />; const cfg = t ? TABLE_TYPES[t.table_type] : null; const hasRes = t && getTableRes(t.id); const isSelected = t && selectedTables.has(t.id); return (<div key={`${row}-${col}`} onClick={(e) => { if (tableSelectMode && t) { toggleTableSelection(t.id, e) } else if (forRes) { t && handleTableClickForRes(t) } else if (!t) { handleCellClick(row, col) } }} style={{ width: cellSize, height: cellSize, backgroundColor: t ? (isSelected ? `${colors.warning}40` : hasRes ? `${colors.error}40` : `${cfg.color}25`) : 'rgba(255,255,255,0.03)', border: `2px solid ${t ? (isSelected ? colors.warning : hasRes ? colors.error : cfg.color) : 'transparent'}`, borderRadius: 6, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: forRes ? (t ? 'pointer' : 'default') : 'pointer', position: 'relative' }}>{t && <><span style={{ fontSize: 9, fontWeight: 700, color: isSelected ? colors.warning : hasRes ? colors.error : cfg.color }}>{t.table_number}</span>{hasRes && <span style={{ fontSize: 8, color: colors.error }}>📋</span>}{tableSelectMode && <div style={{ position: 'absolute', top: 2, left: 2, width: 12, height: 12, borderRadius: 3, border: `2px solid ${isSelected ? colors.warning : colors.border}`, backgroundColor: isSelected ? colors.warning : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{isSelected && <span style={{ color: colors.noir, fontSize: 8 }}>✓</span>}</div>}{!forRes && !tableSelectMode && <button onClick={(e) => handleTableClick(e, t)} style={{ position: 'absolute', top: -6, right: -6, width: 16, height: 16, borderRadius: '50%', border: 'none', backgroundColor: colors.error, color: '#fff', fontSize: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>}</>}</div>) }))}</div>
        
        {/* Stage indicator for back zone */}
        {activeZone === 'back' && <div style={{...stageStyle, marginTop: 8, marginBottom: 0}}>🎪 SCENĂ</div>}
        
        {!forRes && <div style={{ marginTop: 12, display: 'flex', gap: 8 }}><button onClick={addRow} style={{...s.btnSm, backgroundColor: 'transparent', border: `1px solid ${colors.border}`, color: colors.textMuted}}>+ Rând</button><button onClick={addCol} style={{...s.btnSm, backgroundColor: 'transparent', border: `1px solid ${colors.border}`, color: colors.textMuted}}>+ Coloană</button></div>}
      </div>
    )
  }
  ...
  return (
    <div style={s.container}>
      ...
      <div style={s.content}>
        ...
        {activeTab === 'menu' && (<><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}><div style={s.title}>Meniu ({menuItems.length} produse)</div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><button onClick={() => setSelectMode(!selectMode)} style={{...s.btnSm, backgroundColor: selectMode ? colors.champagne : 'transparent', color: selectMode ? colors.noir : colors.textMuted, border: `1px solid ${selectMode ? colors.champagne : colors.border}`}}>{selectMode ? '✓ Selectare' : '☐ Selectare'}</button><button onClick={openCategoryOrderModal} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>📑 Ordine</button><button onClick={handleCreateCategory} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>+ Cat</button><button onClick={() => setShowMenuTemplateModal(true)} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.champagne, border: `1px solid ${colors.champagne}`}}>💾</button><button onClick={() => { setEditingProduct(null); setProductForm({ name: '', description: '', default_price: '', category_id: categories[0]?.id || '', product_type: 'cocktail', badge: '', is_available: true }); setShowProductModal(true) }} style={{...s.btn, backgroundColor: colors.champagne, color: colors.noir}}>+ Produs</button></div></div>{selectMode && selectedItems.size > 0 && (<div style={{ display: 'flex', gap: 8, marginBottom: 16, padding: 12, backgroundColor: `${colors.champagne}15`, borderRadius: 8, flexWrap: 'wrap', alignItems: 'center' }}><span style={{ fontSize: 12, color: colors.champagne, fontWeight: 600 }}>{selectedItems.size} selectate</span><div style={{ flex: 1 }} /><button onClick={() => setShowBulkPriceModal(true)} style={{...s.btnSm, backgroundColor: colors.warning, color: colors.noir}}>💰</button><button onClick={() => handleBulkToggleStock(true)} style={{...s.btnSm, backgroundColor: colors.success, color: '#fff'}}>✓</button><button onClick={() => handleBulkToggleStock(false)} style={{...s.btnSm, backgroundColor: colors.textMuted, color: '#fff'}}>✗</button><button onClick={handleBulkDelete} style={{...s.btnSm, backgroundColor: colors.error, color: '#fff'}}>🗑️</button><button onClick={() => { setSelectedItems(new Set()); setSelectMode(false) }} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>×</button></div>)}{categories.map(cat => { const catItems = menuItems.filter(m => m.categories?.name?.toLowerCase() === cat.name.toLowerCase()).sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0)); const allCatSelected = catItems.length > 0 && catItems.every(m => selectedItems.has(m.id)); return (<div key={cat.id} style={{ marginBottom: 24 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, padding: '8px 12px', backgroundColor: `${colors.champagne}10`, borderRadius: 6 }}><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>{selectMode && (<div onClick={() => selectAllInCategory(cat.name)} style={{...s.checkbox, borderColor: allCatSelected ? colors.champagne : colors.border, backgroundColor: allCatSelected ? colors.champagne : 'transparent'}}>{allCatSelected && <span style={{ color: colors.noir, fontSize: 12 }}>✓</span>}</div>)}<div style={{ fontSize: 14, fontWeight: 600, color: colors.champagne }}>{cat.name}</div><span style={{ fontSize: 11, color: colors.textMuted }}>({catItems.length})</span></div><div style={{ display: 'flex', gap: 8 }}><button onClick={() => handleEditCategory(cat)} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 12, cursor: 'pointer' }}>✏️</button><button onClick={() => handleDeleteCategory(cat.id)} style={{ background: 'none', border: 'none', color: colors.error, fontSize: 12, cursor: 'pointer', opacity: 0.6 }}>✕</button></div></div>{catItems.map((item, idx) => (<div key={item.id} style={{...s.card, opacity: item.is_available ? 1 : 0.5, display: 'flex', alignItems: 'center', gap: 12 }}>{selectMode && (<div onClick={() => toggleItemSelection(item.id)} style={{...s.checkbox, borderColor: selectedItems.has(item.id) ? colors.champagne : colors.border, backgroundColor: selectedItems.has(item.id) ? colors.champagne : 'transparent'}}>{selectedItems.has(item.id) && <span style={{ color: colors.noir, fontSize: 12 }}>✓</span>}</div>)}{!selectMode && (<div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}><button onClick={() => handleMoveProduct(item, 'up')} disabled={idx === 0} style={{ background: 'none', border: 'none', color: idx === 0 ? colors.border : colors.textMuted, fontSize: 10, cursor: idx === 0 ? 'default' : 'pointer', padding: 2 }}>▲</button><button onClick={() => handleMoveProduct(item, 'down')} disabled={idx === catItems.length - 1} style={{ background: 'none', border: 'none', color: idx === catItems.length - 1 ? colors.border : colors.textMuted, fontSize: 10, cursor: idx === catItems.length - 1 ? 'default' : 'pointer', padding: 2 }}>▼</button></div>)}<div style={{ flex: 1 }}><div style={{ fontWeight: 500, marginBottom: 4 }}>{item.name}{item.badge && <span style={{...s.badge, backgroundColor: item.badge === 'popular' ? colors.error : item.badge === 'premium' ? colors.champagne : item.badge === 'new' ? colors.success : colors.normal, color: item.badge === 'premium' ? colors.noir : '#fff' }}>{item.badge}</span>}{!item.is_available && <span style={{...s.badge, backgroundColor: colors.textMuted }}>STOC 0</span>}</div>{item.description && <div style={{ fontSize: 12, color: colors.textMuted }}>{item.description}</div>}<div style={{ fontSize: 11, color: colors.textMuted, marginTop: 4 }}>{item.product_type || 'other'}</div></div><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ fontSize: 15, color: colors.champagne, fontWeight: 500, marginRight: 8 }}>{item.default_price} LEI</div>{!selectMode && <><button onClick={() => handleToggleStock(item)} style={{...s.btnSm, backgroundColor: 'transparent', color: item.is_available ? colors.success : colors.error, border: `1px solid ${item.is_available ? colors.success : colors.error}`, padding: '4px 8px' }}>{item.is_available ? '✓' : '✗'}</button><button onClick={() => openEditProduct(item)} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.champagne, border: `1px solid ${colors.champagne}`, padding: '4px 8px' }}>✏️</button><button onClick={() => handleDeleteProduct(item.id)} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.error, border: `1px solid ${colors.error}`, padding: '4px 8px' }}>✕</button></>}</div></div>))}</div>) })}{menuTemplates?.length > 0 && (<div style={{ marginTop: 24 }}><div style={s.title}>Template-uri meniu</div>{menuTemplates.map(t => (<div key={t.id} style={{...s.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><div><div style={{ fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8 }}>{t.name}{defaultTemplateId === t.id && <span style={{...s.badge, backgroundColor: colors.success, color: '#fff'}}>DEFAULT</span>}</div><div style={{ fontSize: 11, color: colors.textMuted }}>{t.menu_config?.length || 0} produse</div></div><div style={{ display: 'flex', gap: 8 }}><button onClick={() => handleSetDefaultTemplate(t.id)} style={{...s.btnSm, backgroundColor: defaultTemplateId === t.id ? colors.success : 'transparent', color: defaultTemplateId === t.id ? '#fff' : colors.success, border: `1px solid ${colors.success}`}}>{defaultTemplateId === t.id ? '✓' : '☆'}</button><button onClick={() => handleDeleteMenuTemplate(t.id)} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.error, border: `1px solid ${colors.error}`}}>✕</button></div></div>))}</div>)}</>)}
        {activeTab === 'qr' && selectedEvent && (<><div style={s.title}>QR Codes: {selectedEvent.name}</div><div style={{...s.card, marginBottom: 16, backgroundColor: `${colors.champagne}10`, border: `1px solid ${colors.champagne}`}}><div style={{ fontSize: 12, color: colors.champagne, marginBottom: 8, fontWeight: 600 }}>📱 Link-uri pentru mese</div><div style={{ fontSize: 11, color: colors.textMuted }}>🟢 <strong>LIVE</strong> - subdomeniu.sip-app.ro/MASA</div><div style={{ fontSize: 11, color: colors.textMuted, marginTop: 4 }}>🧪 <strong>TEST</strong> - pentru testare locală</div></div><div style={{...s.card, marginBottom: 16}}><label style={s.label}>Subdomeniu venue (ex: nuba, intooit)</label><input id="venueSubdomain" defaultValue="nuba" style={{...s.input, marginBottom: 0}} placeholder="nuba" /></div>{eventTables.map(t => { const tableSlug = t.table_number.replace(/\s+/g, ''); const getLiveUrl = () => { const subdomain = document.getElementById('venueSubdomain')?.value || 'nuba'; return `https://${subdomain}.sip-app.ro/${tableSlug}`; }; const testUrl = `${window.location.origin}/m/testing/${tableSlug}`; return (<div key={t.id} style={{...s.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><div style={{ flex: 1 }}><div style={{ fontWeight: 500, marginBottom: 6 }}>{t.table_number}</div><div style={{ fontSize: 11, color: colors.success, marginBottom: 2 }}>🟢 [subdomain].sip-app.ro/{tableSlug}</div><div style={{ fontSize: 11, color: colors.warning }}>🧪 /m/testing/{tableSlug}</div></div><div style={{ display: 'flex', gap: 8 }}><button onClick={() => { navigator.clipboard.writeText(getLiveUrl()); alert('Link LIVE copiat: ' + getLiveUrl()) }} style={{...s.btnSm, backgroundColor: colors.success, color: '#fff'}}>LIVE</button><button onClick={() => { navigator.clipboard.writeText(testUrl); alert('Link TEST copiat!') }} style={{...s.btnSm, backgroundColor: colors.warning, color: colors.noir}}>TEST</button></div></div>) })}</>)}
        {activeTab === 'waiters' && (<><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}><div style={s.title}>Staff</div><button onClick={() => setShowWaiterModal(true)} style={{...s.btn, backgroundColor: colors.champagne, color: colors.noir}}>+ Ospătar</button></div><div style={s.filterRow}>{['active', 'deleted', 'all'].map(f => (<button key={f} onClick={() => setWaiterFilter(f)} style={{...s.filterBtn, backgroundColor: waiterFilter === f ? colors.champagne : 'transparent', color: waiterFilter === f ? colors.noir : colors.textMuted}}>{f === 'active' ? 'Activi' : f === 'deleted' ? 'Șterși' : 'Toți'}</button>))}</div>{waiters.map(w => (<div key={w.id} onClick={() => loadWaiterStats(w)} style={{...s.card, cursor: 'pointer', opacity: w.is_active ? 1 : 0.5}}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div style={{ flex: 1 }}><div style={{ fontWeight: 500, marginBottom: 4 }}>{w.name}</div><div style={{ fontSize: 12, color: colors.textMuted, marginBottom: 4 }}>{w.phone}</div><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 11, color: colors.textMuted }}>PIN:</span><span style={{ fontSize: 12, fontFamily: 'monospace', color: colors.champagne, letterSpacing: 2 }}>{showPin[w.id] ? (w.pin || '----') : '••••'}</span><button onClick={(e) => { e.stopPropagation(); toggleShowPin(w.id) }} style={{ background: 'none', border: 'none', fontSize: 12, cursor: 'pointer', color: colors.textMuted }}>{showPin[w.id] ? '🙈' : '👁️'}</button></div></div><div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><button onClick={(e) => openEditWaiter(w, e)} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.champagne, border: `1px solid ${colors.champagne}`}}>✏️</button>{selectedEvent && (<button onClick={(e) => { e.stopPropagation(); handleToggleWaiterEvent(w.id) }} style={{...s.btnSm, backgroundColor: eventWaiters.some(ew => ew.waiter_id === w.id) ? colors.success : 'transparent', color: eventWaiters.some(ew => ew.waiter_id === w.id) ? '#fff' : colors.textMuted, border: `1px solid ${eventWaiters.some(ew => ew.waiter_id === w.id) ? colors.success : colors.border}`}}>{eventWaiters.some(ew => ew.waiter_id === w.id) ? '✓ Activ' : 'Adaugă'}</button>)}{w.is_active ? (<button onClick={(e) => handleDeleteWaiter(e, w.id)} style={{...s.btnSm, backgroundColor: 'transparent', color: colors.error, border: `1px solid ${colors.error}`}}>✕</button>) : (<button onClick={(e) => { e.stopPropagation(); handleRestoreWaiter(w.id) }} style={{...s.btnSm, backgroundColor: colors.success, color: '#fff'}}>↩️</button>)}</div></div></div>))}{selectedEvent && eventTables.length > 0 && (<div style={{ marginTop: 32 }}><div style={s.title}>Atribuire mese: {selectedEvent.name}</div>{renderStaffGrid()}</div>)}</>)}
        {activeTab === 'customers' && (<><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}><div style={s.title}>Clienți ({crmCustomers.length})</div><button onClick={loadCRMCustomers} style={{...s.btnSm, backgroundColor: colors.champagne, color: colors.noir}}>🔄 Refresh</button></div>{crmCustomers.length === 0 ? (<div style={{ textAlign: 'center', padding: 48, color: colors.textMuted }}><div style={{ fontSize: 48, marginBottom: 16 }}>👥</div><p>Niciun client încă</p><p style={{ fontSize: 12 }}>Clienții apar din rezervări</p></div>) : crmCustomers.map((c, idx) => (<div key={idx} style={{...s.card, cursor: 'pointer'}} onClick={() => { setSelectedCustomer(c); setShowCustomerModal(true) }}><div style={{ display: 'flex', alignItems: 'center', gap: 16 }}><div style={{ width: 48, height: 48, borderRadius: '50%', backgroundColor: c.isVip ? `${colors.vip}30` : `${colors.normal}20`, border: `2px solid ${c.isVip ? colors.vip : colors.normal}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>{c.isVip ? '⭐' : '👤'}</div><div style={{ flex: 1 }}><div style={{ fontWeight: 500 }}>{c.name} {c.isVip && <span style={{...s.badge, backgroundColor: colors.vip, color: colors.noir}}>VIP</span>}</div><div style={{ fontSize: 11, color: colors.textMuted }}>{c.phone || ''} • {c.eventCount || 0} evenimente</div></div><div style={{ textAlign: 'right' }}><div style={{ fontSize: 18, fontWeight: 600, color: colors.champagne }}>{(c.totalSpend || 0).toLocaleString()}</div><div style={{ fontSize: 10, color: colors.textMuted }}>LEI</div></div></div></div>))}</>)}
      </div>

      {showEventModal && <div style={s.modal} onClick={closeEventModal}><div style={s.modalBox} onClick={e => e.stopPropagation()}><div style={s.modalHead}><span style={{ fontSize: 16, fontWeight: 600 }}>{editingEvent ? 'Editează' : 'Eveniment nou'}</span><button onClick={closeEventModal} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 20, cursor: 'pointer' }}>✕</button></div><div style={s.modalBody}><label style={s.label}>Nume</label><input value={eventForm.name} onChange={e => setEventForm({...eventForm, name: e.target.value})} style={s.input} /><label style={s.label}>Data</label><input type="date" value={eventForm.event_date} onChange={e => setEventForm({...eventForm, event_date: e.target.value})} style={s.input} /><label style={s.label}>Ora</label><input type="time" value={eventForm.start_time} onChange={e => setEventForm({...eventForm, start_time: e.target.value})} style={s.input} />{!editingEvent && layoutTemplates.length > 0 && <><label style={s.label}>Layout</label><select value={selectedTemplate || ''} onChange={e => setSelectedTemplate(e.target.value || null)} style={s.select}><option value="">Fără</option>{layoutTemplates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></>}{!editingEvent && menuTemplates.length > 0 && <><label style={s.label}>Meniu</label><select value={selectedMenuTemplate || ''} onChange={e => setSelectedMenuTemplate(e.target.value || null)} style={s.select}><option value="">{defaultTemplateId ? 'Default' : 'Fără'}</option>{menuTemplates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></>}</div><div style={s.modalFoot}><button onClick={closeEventModal} style={{...s.btn, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>Anulează</button><button onClick={editingEvent ? handleUpdateEvent : handleCreateEvent} style={{...s.btn, backgroundColor: colors.champagne, color: colors.noir}}>{editingEvent ? 'Salvează' : 'Creează'}</button></div></div></div>}
      {showWaiterModal && <div style={s.modal} onClick={() => { setShowWaiterModal(false); setEditingWaiter(null); setWaiterForm({ name: '', phone: '', pin: '' }) }}><div style={s.modalBox} onClick={e => e.stopPropagation()}><div style={s.modalHead}><span style={{ fontSize: 16, fontWeight: 600 }}>{editingWaiter ? 'Editează ospătar' : 'Ospătar nou'}</span><button onClick={() => { setShowWaiterModal(false); setEditingWaiter(null); setWaiterForm({ name: '', phone: '', pin: '' }) }} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 20, cursor: 'pointer' }}>✕</button></div><div style={s.modalBody}><label style={s.label}>Nume</label><input value={waiterForm.name} onChange={e => setWaiterForm({...waiterForm, name: e.target.value})} style={s.input} placeholder="Ion Popescu" /><label style={s.label}>Telefon</label><input value={waiterForm.phone} onChange={e => setWaiterForm({...waiterForm, phone: e.target.value})} style={s.input} placeholder="0722123456" /><label style={s.label}>PIN (4 cifre) {!editingWaiter && <span style={{ color: colors.textMuted, fontWeight: 400 }}>- se generează automat dacă lași gol</span>}</label><input value={waiterForm.pin} onChange={e => setWaiterForm({...waiterForm, pin: e.target.value.replace(/\D/g, '').slice(0, 4)})} style={s.input} placeholder="1234" maxLength={4} />{editingWaiter && <button onClick={() => setWaiterForm({...waiterForm, pin: generatePin()})} style={{...s.btnSm, marginTop: 8, backgroundColor: 'transparent', color: colors.champagne, border: `1px solid ${colors.champagne}`}}>🔄 Generează PIN nou</button>}</div><div style={s.modalFoot}><button onClick={() => { setShowWaiterModal(false); setEditingWaiter(null); setWaiterForm({ name: '', phone: '', pin: '' }) }} style={{...s.btn, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>Anulează</button><button onClick={editingWaiter ? handleUpdateWaiter : handleCreateWaiter} style={{...s.btn, backgroundColor: colors.champagne, color: colors.noir}}>{editingWaiter ? 'Salvează' : 'Adaugă'}</button></div></div></div>}
      {showReservationModal && selectedTableForRes && <div style={s.modal} onClick={() => { setShowReservationModal(false); setEditingReservation(null) }}><div style={s.modalBox} onClick={e => e.stopPropagation()}><div style={s.modalHead}><span style={{ fontSize: 16, fontWeight: 600 }}>{editingReservation ? 'Editează' : 'Rezervare'}: {selectedTableForRes.table_number}</span><button onClick={() => { setShowReservationModal(false); setEditingReservation(null) }} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 20, cursor: 'pointer' }}>✕</button></div><div style={s.modalBody}><label style={s.label}>Nume</label><input value={reservationForm.customer_name} onChange={e => setReservationForm({...reservationForm, customer_name: e.target.value})} style={s.input} /><label style={s.label}>Telefon</label><input value={reservationForm.customer_phone} onChange={e => setReservationForm({...reservationForm, customer_phone: e.target.value})} style={s.input} /><label style={s.label}>Persoane</label><input type="number" value={reservationForm.party_size} onChange={e => setReservationForm({...reservationForm, party_size: parseInt(e.target.value)})} style={s.input} /><label style={s.label}>Ora</label><input type="time" value={reservationForm.reservation_time} onChange={e => setReservationForm({...reservationForm, reservation_time: e.target.value})} style={s.input} /><label style={s.label}>Note</label><input value={reservationForm.notes} onChange={e => setReservationForm({...reservationForm, notes: e.target.value})} style={s.input} /><label style={{ display: 'flex', alignItems: 'center', gap: 8 }}><input type="checkbox" checked={reservationForm.is_vip} onChange={e => setReservationForm({...reservationForm, is_vip: e.target.checked})} /><span style={{ fontSize: 13, color: colors.textMuted }}>VIP</span></label></div><div style={s.modalFoot}>{editingReservation && <button onClick={() => { handleDeleteRes(editingReservation.id); setShowReservationModal(false); setEditingReservation(null) }} style={{...s.btn, backgroundColor: colors.error, color: '#fff'}}>🗑️ Șterge</button>}<div style={{flex:1}}/><button onClick={() => { setShowReservationModal(false); setEditingReservation(null) }} style={{...s.btn, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>Anulează</button><button onClick={editingReservation ? handleUpdateRes : handleCreateRes} style={{...s.btn, backgroundColor: colors.champagne, color: colors.noir}}>{editingReservation ? 'Salvează' : 'Rezervă'}</button></div></div></div>}
      {showProductModal && <div style={s.modal} onClick={closeProductModal}><div style={s.modalBox} onClick={e => e.stopPropagation()}><div style={s.modalHead}><span style={{ fontSize: 16, fontWeight: 600 }}>{editingProduct ? 'Editează' : 'Produs nou'}</span><button onClick={closeProductModal} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 20, cursor: 'pointer' }}>✕</button></div><div style={s.modalBody}><label style={s.label}>Nume</label><input value={productForm.name} onChange={e => setProductForm({...productForm, name: e.target.value})} style={s.input} /><label style={s.label}>Descriere</label><input value={productForm.description} onChange={e => setProductForm({...productForm, description: e.target.value})} style={s.input} /><label style={s.label}>Preț</label><input type="number" value={productForm.default_price} onChange={e => setProductForm({...productForm, default_price: e.target.value})} style={s.input} /><label style={s.label}>Categorie</label><select value={productForm.category_id} onChange={e => setProductForm({...productForm, category_id: e.target.value})} style={s.select}><option value="">-</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select><label style={s.label}>Tip</label><select value={productForm.product_type} onChange={e => setProductForm({...productForm, product_type: e.target.value})} style={s.select}>{PRODUCT_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select><label style={s.label}>Badge</label><select value={productForm.badge} onChange={e => setProductForm({...productForm, badge: e.target.value})} style={s.select}><option value="">-</option>{BADGES.map(b => <option key={b} value={b}>{b}</option>)}</select></div><div style={s.modalFoot}><button onClick={closeProductModal} style={{...s.btn, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>Anulează</button><button onClick={editingProduct ? handleUpdateProduct : handleCreateProduct} style={{...s.btn, backgroundColor: colors.champagne, color: colors.noir}}>{editingProduct ? 'Salvează' : 'Adaugă'}</button></div></div></div>}
      ...
      {showBulkPriceModal && <div style={s.modal} onClick={() => setShowBulkPriceModal(false)}><div style={s.modalBox} onClick={e => e.stopPropagation()}><div style={s.modalHead}><span style={{ fontSize: 16, fontWeight: 600 }}>Modifică prețuri</span><button onClick={() => setShowBulkPriceModal(false)} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 20, cursor: 'pointer' }}>✕</button></div><div style={s.modalBody}><div style={{ textAlign: 'center', padding: 12, marginBottom: 16, backgroundColor: `${colors.warning}15`, borderRadius: 8 }}><div style={{ fontSize: 14, color: colors.warning }}>{selectMode && selectedItems.size > 0 ? `${selectedItems.size} produse` : `${menuItems.length} produse`}</div></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}><button onClick={() => setBulkPriceChange({...bulkPriceChange, type: 'percent'})} style={{...s.btn, backgroundColor: bulkPriceChange.type === 'percent' ? colors.champagne : 'transparent', color: bulkPriceChange.type === 'percent' ? colors.noir : colors.textMuted, border: `1px solid ${colors.border}`}}>%</button><button onClick={() => setBulkPriceChange({...bulkPriceChange, type: 'fixed'})} style={{...s.btn, backgroundColor: bulkPriceChange.type === 'fixed' ? colors.champagne : 'transparent', color: bulkPriceChange.type === 'fixed' ? colors.noir : colors.textMuted, border: `1px solid ${colors.border}`}}>LEI</button></div><label style={s.label}>{bulkPriceChange.type === 'percent' ? 'Procent' : 'Sumă'}</label><input type="number" value={bulkPriceChange.value} onChange={e => setBulkPriceChange({...bulkPriceChange, value: parseFloat(e.target.value) || 0})} style={s.input} /></div><div style={s.modalFoot}><button onClick={() => setShowBulkPriceModal(false)} style={{...s.btn, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>Anulează</button><button onClick={handleBulkPrice} style={{...s.btn, backgroundColor: colors.warning, color: colors.noir}}>Aplică</button></div></div></div>}
      {showCategoryOrderModal && <div style={s.modal} onClick={() => setShowCategoryOrderModal(false)}><div style={s.modalBox} onClick={e => e.stopPropagation()}><div style={s.modalHead}><span style={{ fontSize: 16, fontWeight: 600 }}>Ordine categorii</span><button onClick={() => setShowCategoryOrderModal(false)} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 20, cursor: 'pointer' }}>✕</button></div><div style={s.modalBody}>{tempCategoryOrder.map((cat, idx) => (<div key={cat.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, marginBottom: 8, backgroundColor: colors.noir, borderRadius: 6, border: `1px solid ${colors.border}` }}><div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}><button onClick={() => moveCategoryInOrder(idx, 'up')} disabled={idx === 0} style={{ background: 'none', border: 'none', color: idx === 0 ? colors.border : colors.champagne, fontSize: 14, cursor: idx === 0 ? 'default' : 'pointer', padding: 2 }}>▲</button><button onClick={() => moveCategoryInOrder(idx, 'down')} disabled={idx === tempCategoryOrder.length - 1} style={{ background: 'none', border: 'none', color: idx === tempCategoryOrder.length - 1 ? colors.border : colors.champagne, fontSize: 14, cursor: idx === tempCategoryOrder.length - 1 ? 'default' : 'pointer', padding: 2 }}>▼</button></div><span style={{ flex: 1, fontWeight: 500 }}>{cat.name}</span><span style={{ fontSize: 11, color: colors.textMuted }}>{menuItems.filter(m => m.category_id === cat.id).length}</span></div>))}</div><div style={s.modalFoot}><button onClick={() => setShowCategoryOrderModal(false)} style={{...s.btn, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>Anulează</button><button onClick={saveCategoryOrder} style={{...s.btn, backgroundColor: colors.champagne, color: colors.noir}}>Salvează</button></div></div></div>}
      {showCategoryEditModal && editingCategory && <div style={s.modal} onClick={() => setShowCategoryEditModal(false)}><div style={s.modalBox} onClick={e => e.stopPropagation()}><div style={s.modalHead}><span style={{ fontSize: 16, fontWeight: 600 }}>Editează categorie</span><button onClick={() => setShowCategoryEditModal(false)} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 20, cursor: 'pointer' }}>✕</button></div><div style={s.modalBody}><label style={s.label}>Nume</label><input value={categoryName} onChange={e => setCategoryName(e.target.value)} style={s.input} /></div><div style={s.modalFoot}><button onClick={() => setShowCategoryEditModal(false)} style={{...s.btn, backgroundColor: 'transparent', color: colors.textMuted, border: `1px solid ${colors.border}`}}>Anulează</button><button onClick={handleUpdateCategory} style={{...s.btn, backgroundColor: colors.champagne, color: colors.noir}}>Salvează</button></div></div></div>}
      ...
      {showAssignModal && selectedTableForAssign && <div style={s.modal} onClick={() => setShowAssignModal(false)}><div style={s.modalBox} onClick={e => e.stopPropagation()}><div style={s.modalHead}><span style={{ fontSize: 16, fontWeight: 600 }}>Atribuie: {selectedTableForAssign.table_number}</span><button onClick={() => setShowAssignModal(false)} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 20, cursor: 'pointer' }}>✕</button></div><div style={s.modalBody}>{activeWaitersOnEvent.length === 0 ? <div style={{ textAlign: 'center', padding: 24, color: colors.textMuted }}>Nu ai ospătari activi pe acest eveniment</div> : activeWaitersOnEvent.map(w => { const isAssigned = tableAssignments.find(a => a.event_table_id === selectedTableForAssign.id)?.waiters?.id === w.id; return (<div key={w.id} onClick={() => handleAssignWaiterToTable(w.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 14, marginBottom: 8, backgroundColor: isAssigned ? `${waiterColorMap[w.id]}30` : colors.noir, border: `2px solid ${isAssigned ? waiterColorMap[w.id] : colors.border}`, borderRadius: 8, cursor: 'pointer' }}><div style={{ width: 16, height: 16, borderRadius: '50%', backgroundColor: waiterColorMap[w.id] }} /><span style={{ flex: 1, fontWeight: 500 }}>{w.name}</span>{isAssigned && <span style={{ color: waiterColorMap[w.id], fontWeight: 600 }}>✓</span>}</div>) })}{tableAssignments.find(a => a.event_table_id === selectedTableForAssign.id) && <button onClick={handleUnassignTable} style={{...s.btn, width: '100%', marginTop: 8, backgroundColor: 'transparent', color: colors.error, border: `1px solid ${colors.error}`}}>Elimină atribuirea</button>}</div></div></div>}
      {showCustomerModal && selectedCustomer && <div style={s.modal} onClick={() => setShowCustomerModal(false)}><div style={{...s.modalBox, maxWidth: 500}} onClick={e => e.stopPropagation()}><div style={s.modalHead}><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><div style={{ width: 40, height: 40, borderRadius: '50%', backgroundColor: selectedCustomer.isVip ? `${colors.vip}30` : `${colors.normal}20`, border: `2px solid ${selectedCustomer.isVip ? colors.vip : colors.normal}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{selectedCustomer.isVip ? '⭐' : '👤'}</div><div><div style={{ fontWeight: 600 }}>{selectedCustomer.name}</div>{selectedCustomer.isVip && <span style={{...s.badge, backgroundColor: colors.vip, color: colors.noir}}>VIP</span>}</div></div><button onClick={() => setShowCustomerModal(false)} style={{ background: 'none', border: 'none', color: colors.textMuted, fontSize: 20, cursor: 'pointer' }}>✕</button></div><div style={s.modalBody}><div style={{ backgroundColor: colors.noir, padding: 16, borderRadius: 8, marginBottom: 16 }}>{selectedCustomer.phone && <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${colors.border}` }}><span style={{ color: colors.textMuted }}>📱 Telefon</span><a href={`tel:${selectedCustomer.phone}`} style={{ color: colors.champagne, textDecoration: 'none' }}>{selectedCustomer.phone}</a></div>}{selectedCustomer.email && <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${colors.border}` }}><span style={{ color: colors.textMuted }}>✉️ Email</span><span>{selectedCustomer.email}</span></div>}<div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}><span style={{ color: colors.textMuted }}>🎉 Evenimente</span><span>{selectedCustomer.eventCount || 0}</span></div></div><div style={{ textAlign: 'center', padding: 20, backgroundColor: `${colors.champagne}15`, borderRadius: 8, marginBottom: 16 }}><div style={{ fontSize: 11, color: colors.textMuted, letterSpacing: 2, marginBottom: 4 }}>TOTAL SPEND</div><div style={{ fontSize: 32, fontWeight: 300, color: colors.champagne }}>{(selectedCustomer.totalSpend || 0).toLocaleString()} LEI</div></div><div style={{ fontSize: 11, color: colors.textMuted, letterSpacing: 2, marginBottom: 12 }}>ISTORIC REZERVĂRI</div>{selectedCustomer.reservations?.map((res, idx) => (<div key={idx} style={{ padding: 12, backgroundColor: colors.noir, borderRadius: 6, marginBottom: 8, border: `1px solid ${colors.border}` }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}><span style={{ fontWeight: 500 }}>{res.events?.name || 'Event'}</span><span style={{ color: colors.champagne, fontWeight: 600 }}>{(res.tableSpend || 0).toLocaleString()} LEI</span></div><div style={{ fontSize: 11, color: colors.textMuted }}>📅 {res.events?.event_date ? new Date(res.events.event_date).toLocaleDateString('ro-RO') : '-'} • 🪑 {res.event_tables?.table_number || '-'} • 👥 {res.party_size}p</div></div>))}</div></div></div>}
    </div>
  )
}
